...
object LogAnalysis {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var paramets = ParameterTool.fromArgs(args)
    val bootstrapSrver = if (paramets.has("bootstrapServer")) {
     println(s"打印输出参数："+paramets.get("bootstrapServer"))
      paramets.get("bootstrapServer")
    } else {
      "10.199.151.15:9092"
    }
    ...
    properties.setProperty("bootstrap.servers", bootstrapSrver)
    properties.setProperty("group.id", "test-wsk-group—2")
    ...
    val resultData = logData.assignTimestampsAndWatermarks(new AssignerWithPeriodicWatermarks[(Long, String, Long)] {
      ...
      //此类型水印是为，解决 event_time无序的问题，默认：窗口滑动时间长度+maxOutOfOrderness时间内，数据一定会到达
      ...
      .apply(new WindowFunction[(Long, String, Long), (String, String, Long), Tuple, TimeWindow] {
        override def apply(key: Tuple, window: TimeWindow, input: Iterable[(Long, String, Long)], out: Collector[(String, String, Long)]): Unit = {
          ...
          //          val beginingTime = format.format(new Date(times.min))
          //          val endTime = format.format(new Date(times.max))
    ...
    httpHosts.add(new HttpHost("192.168.175.135", 9200, "http"))
